...
TMUX_SOCKET="/tmp/button-tmux/default"
TMUX_CMD="tmux -S $TMUX_SOCKET"

if [ -n "$1" ]; then
  sessions="$1"
else
  sessions=$($TMUX_CMD list-sessions -F '#S' 2>/dev/null | grep '^btn-')
fi
[ -z "$sessions" ] && exit 0

# Send Ctrl+C to interrupt any running command
for s in $sessions; do
  $TMUX_CMD send-keys -t "$s" C-c C-c 2>/dev/null
done
sleep 1

# Send /exit to gracefully close Claude (deregisters remote session)
for s in $sessions; do
  $TMUX_CMD send-keys -t "$s" '/exit' Enter 2>/dev/null
done
sleep 2
